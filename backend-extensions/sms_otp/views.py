...
class EnvoyerOTP(APIView):
    ...
    def post(self, request):
        ...
        if not sent:
            logger.warning(f"SMS non envoyé pour {telephone}, code: {otp.code}")
            # En dev, on retourne le code dans la réponse (à supprimer en prod)
            import os
            if os.environ.get('DJANGO_ENV') == 'development':
                return Response({'detail': 'Code envoyé (dev)', 'code_dev': otp.code})
